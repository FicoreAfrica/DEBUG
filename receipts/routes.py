...
def index():
    ...
    try:
        ...
        return render_template('receipts/index.html', receipts=receipts, format_currency=format_currency, format_date=format_date, t=trans, lang=session.get('lang', 'en'))
...
def add():
    ...
    return render_template('receipts/add.html', form=form, t=trans, lang=session.get('lang', 'en'))
...
def edit(id):
    ...
    try:
        ...
        return render_template('receipts/edit.html', form=form, receipt=receipt, t=trans, lang=session.get('lang', 'en'))
    except Exception as e:
        logger.error(f"Error fetching receipt {id} for user {current_user.id}: {str(e)}")
        flash(trans('receipts_record_not_found', default='Cashflow not found'), 'danger')
        return redirect(url_for('receipts_blueprint.index'))
